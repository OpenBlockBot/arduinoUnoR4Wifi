export default Blockly => {
    ...
    Blockly.Arduino.arduino_display_showImage = function (block) {
        const arg0 = Blockly.Arduino.valueToCode(block, 'VALUE', Blockly.Arduino.ORDER_UNARY_POSTFIX) || '0';
        ...
        const hexArray = arg0.match(/.{32}/g).map(bin => `0x${parseInt(bin, 2).toString(16)
            .padStart(8, '0')}`);

        const code = `matrix.loadFrame((const uint32_t[]){ ${hexArray[0]}, ${hexArray[1]}, ${hexArray[2]} });\n`;
        return code;
    };

    Blockly.Arduino.arduino_display_showImageUntil = function (block) {
        const arg0 = Blockly.Arduino.valueToCode(block, 'VALUE', Blockly.Arduino.ORDER_UNARY_POSTFIX) || '0';
        const arg1 = Blockly.Arduino.valueToCode(block, 'TIME', Blockly.Arduino.ORDER_UNARY_POSTFIX) || '0';
    ...
    Blockly.Arduino.arduino_display_showUntilScrollDone = function (block) {
        const arg0 = Blockly.Arduino.valueToCode(block, 'TEXT', Blockly.Arduino.ORDER_UNARY_POSTFIX) || '0';
        ...
        const code = `matrix.println((const char[]){${arg0}});\nmatrix.endText(SCROLL_LEFT);\n`;
        return code;
    };
    ...
    Blockly.Arduino.arduino_display_clearDisplay = function () {
        ...
        const code = 'matrix.clear();\n';
        return code;
    };

    Blockly.Arduino.arduino_display_lightPixelAt = function (block) {
        const sta = block.getFieldValue('STATE');
        const x = Blockly.Arduino.valueToCode(block, 'X', Blockly.Arduino.ORDER_UNARY_POSTFIX) || '0';
        const y = Blockly.Arduino.valueToCode(block, 'Y', Blockly.Arduino.ORDER_UNARY_POSTFIX) || '0';
